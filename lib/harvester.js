...
HarvesterManager.prototype.refresh = function() {
    ...
    var logMissingOrgas = function(results) {
        var missing = [];
        for (var k in results) {
            var r = results[k];
            if (!r.orga || r.orga === "" || r.orga === "ckan-error" || r.orga === "unknown") {
                var tmp = {};
                tmp.id = r.package;
                tmp.message = r.orga;
                missing.push(tmp);
        ...
        if (missing.length === 0) {
            log.info("got organization for every package!");
            return;
        }
        
        var now = moment().format("YYYY-MM-DD_HH-mm-ss");
        writer.write("./missing-orga-"+now+".json", missing);
    };
...
HarvesterManager.prototype.resolveOrganizations = function(servicesWithPackage) {
    ...
    var bb = Bluebird.map(urls, function(key) {
        ...
        return baseRequest({url: self.config.ckan.url + '/api/3/action/package_show?id=' + package}).then(function(body) {
            var result = {};
            result.url = key;
            ...
            log.error("error on resolving CKAN package", error);
            var result = {};
            result.url = key;
